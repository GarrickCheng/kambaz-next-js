...
export default function KambazNavigation() {
    ...
    return (
        <ListGroup className="rounded-0 position-fixed bottom-0 top-0
            ...
            {links.map((link) => {
                // Special handling: when on /Dashboard, only highlight Dashboard (not Courses)
                let isActive;
                ...
                return (
                    <ListGroupItem key={`${link.path}-${link.label}`} 
                        className={`border-0 text-center p-0 ${
                            isActive ? "bg-white" : "bg-black"
                        }`}>
                        <Link href={link.path} id={link.id}
                            className={`d-block text-decoration-none py-3 px-2 ${
                                isActive ? "text-danger" : "text-white"
                            }`}>
                            <Icon className={`fs-3 ${isActive ? "text-danger" : "text-danger"}`} />
                            <br />{link.label}
                        </Link>
                    </ListGroupItem>
                );
